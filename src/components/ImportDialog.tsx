  ...
  return (
    <dialog ref={dialogRef} className="app-dialog import-dialog" onClose={onClose} onCancel={(event) => { event.preventDefault(); onClose(); }}>
      <div className="dialog-header">
        <div className="dialog-title"><span className="dialog-title__icon"><FileUp size={18} /></span><div><h2>导入流程图</h2><p>结构与原貌分级转换</p></div></div>
      ...
      <div className="import-dialog__body">
        ...
        >
          ...
          <strong>{status === 'reading' ? '正在解析文件' : '选择或拖入文件'}</strong>
          <span>单个文件，最大取决于浏览器内存</span>
          <input
            type="file"
      ...
      <div className="dialog-footer">
        ...
        <button className="primary-button" onClick={apply} disabled={!result}>导入到画布</button>
      </div>
    </dialog>
  );
}
